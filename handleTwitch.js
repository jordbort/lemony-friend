...
async function authorizeToken(chatroom, user, args) {
    if (settings.debug) { console.log(`${boldTxt}> authorizeToken(chatroom: ${chatroom}, user: ${user}, args: ${args})${resetTxt}`) }
    const authCodePattern = /^http:\/\/localhost:3000\/\?code=([a-z0-9]*)&scope=.*$/
    if (!authCodePattern.test(args[0])) { return talk(chatroom, `${users[user].displayName}, when using the !authorize command, please paste the whole URL after!`) }
    const authCode = args[0].split(authCodePattern)[1]
